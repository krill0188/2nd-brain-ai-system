...
> **2026-08-01 조사 후 보류 (마스터 결정).** 일방향 알림(1번)은 이미
> `research-run.sh`의 `report_master()`가 `hermes send -t telegram`으로
> 구현·사용 중. 양방향(2번, 텔레그램 답장으로 승인)은 실제 구현 방법을
> 확인하지 못했다 — `hermes hooks`/`hermes webhook` 서브커맨드는 존재하나
> 인바운드 메시지를 스크립트로 라우팅하는 구체적 설정 예제를 찾지 못했고,
> "AI Fleet HITL 승인게이트(2026-07-06)" 재사용 후보도 실제 파일 위치를
> 찾지 못함. 마스터의 실제 운영 중인 텔레그램 봇 인프라에 불확실한 상태로
> 훅을 걸면 다른 정상 기능(헬스체크·브리핑)에 영향을 줄 위험이 있어,
> **CLI 승인 경로(이미 완전히 검증·작동)를 유지**하고 이 항목은 마스터가
> 구체적 위치를 지시하거나 리스크를 감수하고 진행을 요청할 때 재개한다.
