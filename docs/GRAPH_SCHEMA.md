...
## 5. 정식 온톨로지 스택(Protégé/Jena/HermiT/SHACL) 재검토 임계치 (G5, 2026-08-02)

`ONTOLOGY_GUIDED_GRAPHRAG_PLAN.md`에서 정식 DL(Description Logic) 추론기
도입을 보류하며 남긴 결정 — **그래프 DB 트리거(5,000엣지)와는 별개의
독립 임계치**다(그래프 DB는 "저장/질의 방식" 문제, 정식 온톨로지 스택은
"추론 필요성" 문제라 서로 다른 트리거가 맞다). 아래 중 **하나라도**
충족하면 재검토를 시작한다(선제적으로 다시 논의하지 않는다):

1. **canonical 500페이지 초과** — 지금(139개)의 3.6배. 이 규모부터는
   손으로 만든 `ontology/class-hierarchy.json` 같은 경량 딕셔너리로
   커버하기엔 클래스 간 관계가 스스로 복잡해지기 시작할 가능성이 높다.
2. **엣지 3,000개 초과** — 그래프 DB 트리거(5,000)보다 낮게 잡은 이유:
   추론 필요성은 저장 규모보다 먼저 부담이 된다(질의는 그래프 DB 없이도
   JSON 순회로 되지만, "규칙에 안 맞는 것 찾기"는 수작업 한계에 먼저
   부딪힌다).
3. **정식 추론 부재로 인한 실제 오류가 3건 이상 누적** — 예: 손으로 만든
   `ONTOLOGY_CLASS_MAP`/`class-hierarchy.json`의 수동 관리 실수가 실제
   canonical 오분류나 검색 누락으로 이어진 사례. 숫자가 아니라 사건
   기반 트리거 — 이게 더 직접적인 신호다.

트리거 도달 전에는 이 논의를 재소집하지 않는다. 트리거 도달 시에도 정식
스택을 자동 채택하는 게 아니라 이 절을 갱신해 다시 비교 판단한다(§4와
같은 형식으로).
